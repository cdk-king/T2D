	...
	int ncmdshow)
{
	...
	int sys_min_width = GetSystemMetrics(SM_CXHTHUMB);//以像素为单位的水平滚动条上的滑动块宽度
	...
	while (TRUE)
	{
	 ...
	 // move endpoints of line
		...
		if (x1 < 0 || x1 >= (WINDOW_WIDTH - sys_min_width))
		{
			// invert velocity
			x1v = -x1v;
		...
		if (y1 < 0 || y1 >= (WINDOW_HEIGHT - sys_min_height))
		{
			// invert velocity
			y1v = -y1v;
	 ...
	 // now test second endpoint
		if (x2 < 0 || x2 >= (WINDOW_WIDTH - sys_min_width))
		{
			// invert velocity
			x2v = -x2v;
		...
		if (y2 < 0 || y2 >= (WINDOW_HEIGHT - sys_min_height))
		{
			// invert velocity
			y2v = -y2v;
