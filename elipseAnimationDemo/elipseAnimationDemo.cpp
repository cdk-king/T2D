	...
	int ncmdshow)
{
	...
	while (TRUE)
	{
		...
		int x, y;
		x = GetSystemMetrics(SM_CXHTHUMB);//以像素为单位的水平滚动条上的滑动块宽度
		...
		TextOut(hdc, 20, 20, szBuffer, iLength);
	 ...
	 // first select the black pen and brush into context
		...
		if (ball_x < 0 || ball_x > (WINDOW_WIDTH - 32-x))
		{
		...
		if (ball_y < 0 || ball_y > (WINDOW_HEIGHT - 32- sys_min_height))
		{
